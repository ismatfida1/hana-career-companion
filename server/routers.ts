...
export const appRouter = router({
  ...
  ai: router({
    status: publicProcedure.query(() => ({ providers: configuredAiProviders(), activeProvider: ENV.openaiApiKey ? "openai" : ENV.geminiApiKey ? "gemini" : ENV.forgeApiKey ? "forge" : "none" })),
    ...
    chat: publicProcedure.input(z.object({ message: z.string().trim().min(1).max(4000), conversationId: z.number().int().positive().optional(), memoryEnabled: z.boolean().default(true), history: chatHistorySchema })).mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        throw new TRPCError({ code: "SERVICE_UNAVAILABLE", message: error instanceof Error ? `Hana AI request failed: ${error.message}` : "Hana AI request failed. Check the server API key and provider configuration." });
      }
    }),
  }),
  learner: router({

...
export default function HanaGameFrame({ children, title }: { children: ReactNode; title: string }) {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#101d2b] text-[#fffaf4]">
    ...
    <div className="relative mx-auto max-w-7xl px-4 py-6 sm:px-7 lg:px-10">
      <div className="relative mb-5 min-h-[180px] overflow-hidden rounded-[28px] border border-white/10 bg-white/[.04] shadow-2xl sm:min-h-[220px]"><img src={world.image} alt="" aria-hidden="true" className="absolute inset-0 h-full w-full object-contain p-3 opacity-30 sm:p-5" /><div className="absolute inset-0 bg-gradient-to-r from-[#101d2b]/95 via-[#101d2b]/55 to-[#101d2b]/75" /><div className="relative flex min-h-[180px] items-end px-5 py-7 sm:min-h-[220px] sm:px-7"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#f1c77b]/75">Hana adventure · {world.world}</p><h2 className="mt-2 max-w-3xl font-display text-3xl font-semibold tracking-[-.04em] sm:text-4xl">{title}</h2></div></div></div>
      {children}
    </div>
  </main>;
}

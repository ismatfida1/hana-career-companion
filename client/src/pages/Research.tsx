...
const universities = [
  ["Harvard CS50x", "https://cs50.harvard.edu/x/2026/"],
  ["MIT 6.006 Algorithms", "https://ocw.mit.edu/courses/6-006-introduction-to-algorithms-spring-2020/"],
  ["Stanford CS Courses", "https://cs.stanford.edu/courses"],
  ["UC Berkeley CS Courses", "https://eecs.berkeley.edu/academics/undergraduate/eecs-courses/"],
] as const;
...
export default function Research() {
  ...
  const runSearch = async () => { const clean = query.trim(); if (!clean) return; setLoading(true); try { const response = await fetch(`/api/research?q=${encodeURIComponent(clean)}`); const data = await response.json() as { results?: Result[] }; setResults(data.results ?? []); } catch { setResults([]); } finally { setLoading(false); } };

...
export default function AdventureHome() {
  ...
  return (
    <FantasyBackground
      ...
      className={`min-h-[100svh] bg-center transition-colors duration-700 ${dark ? "saturate-[.82]" : "saturate-[1.12]"}`}
    ...
    >
      <header className="absolute inset-x-0 top-0 z-30 flex items-center justify-between px-5 py-5 sm:px-9">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setIsMuted(value => !value)} className={`inline-flex h-11 w-11 items-center justify-center rounded-full border backdrop-blur-md transition ${dark ? "border-[#f1c77b]/40 bg-[#071722]/55 text-[#f1c77b]" : "border-[#477f74]/25 bg-white/55 text-[#2d665e]"}`} aria-label={isMuted ? "Unmute" : "Mute"}>
            {isMuted ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}
          </button>
          <button type="button" onClick={toggleTheme} className={`inline-flex h-11 items-center gap-2 rounded-full border px-4 text-xs font-bold uppercase tracking-[.14em] shadow-lg backdrop-blur-md transition ${dark ? "border-[#a6a9ff]/35 bg-[#071722]/55 text-[#fff7df]" : "border-[#477f74]/25 bg-white/60 text-[#285c55]"}`} aria-label="Switch between bright and dark mode">
            {dark ? "Dark" : "Bright"}
          </button>
        </div>
      </header>

...
function RoadmapView({ onMission }: { onMission: () => void }) {
  ...
  const personalizedStages = roadmapStages.map((stage, index) => index === 0 ? { ...stage, subtitle: `Start here for ${goal}`, nodes: [currentMission?.title || "What is code?", "Python basics", "Git & GitHub"] } : index === 2 && goal.toLowerCase().includes("ai") ? { ...stage, title: "Web Development for intelligent products", subtitle: "Turn data into useful tools", nodes: ["HTML & CSS", "APIs", "Model basics", "Evaluation"] } : stage);
  const selectedStage = personalizedStages.find(stage => stage.number === selectedWorldNumber) ?? personalizedStages[0];
  return <>

...
import { masterResourceLibrary, roadmapResourceSelections } from "@/data/resourceLibrary";
...
const navItems: { id: View; label: string; icon: typeof House }[] = [
  ...
  { id: "chat", label: "Hana", icon: MessageCircle },
];

const roadmapStages = [
  { number: "01", world: "Origin Village", title: "Computer Science Foundations", subtitle: "Learn the ideas everything else is built on.", state: "current", nodes: ["What is code?", "Python basics", "Git & GitHub"], teaches: "How computers, code, and problem solving fit together.", path: ["Meet the world of code", "Write your first Python steps", "Save your work with Git"], project: "A tiny command-line helper", unlock: "Available now" },
  { number: "02", world: "Code Forge", title: "Programming & Problem Solving", subtitle: "Turn ideas into working solutions.", state: "next", nodes: ["Functions", "Data structures", "Debugging"], teaches: "How to break a big problem into small, testable steps.", path: ["Shape reusable functions", "Choose the right data structure", "Debug with confidence"], project: "A useful personal automation tool", unlock: "Complete Origin Village" },
  { number: "03", world: "Weblands", title: "Web Development", subtitle: "Build something people can use.", state: "locked", nodes: ["HTML & CSS", "JavaScript", "REST APIs"], teaches: "How a web page becomes an interactive product that talks to services.", path: ["Create a welcoming interface", "Add interaction with JavaScript", "Connect your first API"], project: "A polished weather or discovery app", unlock: "Complete Code Forge" },
  { number: "04", world: "Cloudspire", title: "Backend, Cloud & Engineering", subtitle: "Make your creations reliable.", state: "locked", nodes: ["Backend services", "Databases", "Deployment"], teaches: "How products store data, serve users, and stay dependable.", path: ["Design a small backend", "Persist real learner data", "Deploy and observe your service"], project: "A deployed full-stack application", unlock: "Complete Weblands" },
  { number: "05", world: "Summit of Builders", title: "Projects, Career & Opportunities", subtitle: "Make your work visible and meaningful.", state: "locked", nodes: ["Portfolio", "Open source", "Opportunities"], teaches: "How to turn your learning into evidence, stories, and next opportunities.", path: ["Shape your portfolio story", "Share work with others", "Choose a brave next opportunity"], project: "A portfolio-ready capstone", unlock: "Complete Cloudspire" },
];

type OpportunityCard = { id?: number; title: string; org: string; type: string; match: string; deadline: string; tags: string[]; tone: "coral" | "mint" | "lilac"; officialUrl?: string; resources?: RoadmapResource[] };
...
function SimpleHomeView({ onNavigate, onMission }: { onNavigate: (view: View) => void; onMission: () => void }) {
  const { data: missions } = trpc.learner.missions.useQuery();
  const currentMission = missions?.find(mission => mission.state !== "completed") ?? missions?.[0];
  const missionTitle = currentMission?.title || "Learn what an API is";
  const progress = currentMission?.progress ?? 0;
  return <div className="simple-home mx-auto max-w-3xl"><section className="simple-home-hero rounded-[30px] p-6 text-white shadow-[0_24px_70px_rgba(34,77,73,.18)] md:p-10"><div className="flex items-center gap-4"><HanaAvatar mood={progress > 0 ? "encouraging" : "concept"} /><div><p className="text-sm font-semibold text-white/75">Hi, I’m Hana.</p><h1 className="mt-1 font-display text-3xl font-semibold tracking-[-0.04em] md:text-4xl">Ready for one small step?</h1></div></div><div className="mt-10 rounded-[24px] bg-white/12 p-5 backdrop-blur-sm md:p-7"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold uppercase tracking-[0.14em] text-white/70">Today’s mission</p><span className="text-xs font-semibold text-white/70">20 minutes</span></div><h2 className="mt-3 font-display text-2xl font-semibold md:text-3xl">{missionTitle}</h2><p className="mt-3 max-w-xl text-sm leading-6 text-white/75">Today, we’re going to learn how apps talk to each other. I’ll guide you one step at a time.</p><div className="mt-6 h-2 overflow-hidden rounded-full bg-white/20"><div className="h-full rounded-full bg-[#ffd4a8] transition-all" style={{ width: `${progress}%` }} /></div><div className="mt-6 flex flex-wrap items-center gap-3"><Button onClick={onMission} className="rounded-full bg-[#fffaf4] px-6 text-[#315d58] hover:bg-white">{progress > 0 ? "Continue mission" : "Start mission"}<ArrowRight className="ml-2 h-4 w-4" /></Button><span className="text-xs font-semibold text-white/65">{progress}% complete</span></div></div></section><section className="mt-5 rounded-[24px] border border-[#eadfd3] bg-[#fffaf4] p-5 md:p-6"><div className="flex items-center justify-between"><div><p className="eyebrow">Your journey</p><h2 className="mt-2 font-display text-xl font-semibold text-[#2d3c39]">Follow the glowing path.</h2></div><Sparkles className="h-5 w-5 text-[#d99172]" /></div><p className="mt-3 text-sm leading-6 text-[#76695d]">Hana keeps the next idea clear. You can explore the full roadmap whenever you’re ready.</p><button onClick={() => onNavigate("roadmap")} className="mt-5 text-sm font-bold text-[#5d947f]">Open roadmap <ArrowRight className="ml-1 inline h-4 w-4" /></button></section></div>;
}
const worldMeta = {
  roadmap: { eyebrow: "World 01 · Navigation", title: "Follow the glowing path.", description: "A small, staged journey that keeps the next computer-science idea visible without asking you to plan everything at once.", icon: Compass, action: "Continue mission", helper: "You are here · one next step" },
  projects: { eyebrow: "World 02 · Guild workshop", title: "Build proof, one checkpoint at a time.", description: "Turn a concept into a useful project, then keep the evidence that shows what you learned.", icon: Layers3, action: "Open current quest", helper: "Learn → Practice → Build → Prove" },
  opportunities: { eyebrow: "World 03 · Adventurer’s Board", title: "Find a quest with a reason.", description: "See a calm, curated set of opportunities that connect to what you are learning now—not a wall of listings.", icon: Trophy, action: "Explore a recommended quest", helper: "Relevance before volume" },
  chat: { eyebrow: "World 04 · Hana’s study", title: "Ask Hana anything.", description: "A familiar conversation space for simpler explanations, project help, and one clear next step.", icon: MessageCircle, action: "Start with a prompt", helper: "Roadmap + project context" },
} as const;

type WorldKind = keyof typeof worldMeta;

function WorldIntro({ kind, onAction }: { kind: WorldKind; onAction?: () => void }) {
  const meta = worldMeta[kind];
  const Icon = meta.icon;
  return (
    <section className={cn("world-intro", `world-intro-${kind}`)} aria-labelledby={`${kind}-world-title`}>
      <div className="world-intro-orbit" aria-hidden="true" />
      <div className="world-intro-content">
        <div className="world-intro-icon"><Icon className="h-5 w-5" /></div>
        <div>
          <p className="world-intro-eyebrow">{meta.eyebrow}</p>
          <h1 id={`${kind}-world-title`} className="world-intro-title">{meta.title}</h1>
          <p className="world-intro-copy">{meta.description}</p>
          <span className="world-intro-helper"><Sparkles className="h-3.5 w-3.5" /> {meta.helper}</span>
        </div>
      </div>
      {onAction && <button type="button" onClick={onAction} className="world-intro-action">{meta.action}<ArrowRight className="h-4 w-4" /></button>}
    </section>
  );
}

const ROADMAP_VISION_IMAGE = "https://files.manuscdn.com/user_upload_by_module/session_file/310519663907899784/WSprBRcurIacWvbM.png";
const CHAT_VISION_IMAGE = "https://files.manuscdn.com/user_upload_by_module/session_file/310519663907899784/JBuyxmYjToCUHehH.png";

type RoadmapLocation = "spark" | "logic" | "loop" | "algorithm";

const roadmapLocationCopy: Record<RoadmapLocation, { label: string; detail: string; state: string }> = {
  spark: { label: "Spark Garden", detail: "Start with the small idea in front of you: what code is and how it can help you solve a problem.", state: "Available now" },
  logic: { label: "Logic Gateway", detail: "The next chapter turns ideas into decisions, conditions, and reusable steps.", state: "Unlocks after your first mission" },
  loop: { label: "Looping Heights", detail: "Practice repetition only when the current concept feels clear enough to use.", state: "Locked for now" },
...
function RoadmapView({ onMission }: { onMission: () => void }) {
  ...
  return (
    <main className={cn("roadmap-vision", pressed && "roadmap-vision-pressed")} aria-label="Hana Roadmap world">
      <img className="roadmap-vision-art" src={ROADMAP_VISION_IMAGE} alt="A magical floating-island roadmap with a glowing path from Spark Garden toward future learning destinations." />
      <div className="roadmap-vision-vignette" aria-hidden="true" />
      <button type="button" className="roadmap-vision-home" onClick={() => window.history.back()} aria-label="Return to Hana home">← Home</button>
      <div className="roadmap-vision-title" aria-hidden="true"><strong>ROADMAP</strong><span>Your path through the world of code</span></div>
      <div className="roadmap-vision-sr-copy"><h1>Roadmap</h1><p>Your path through the world of code. You are here in Spark Garden, working toward {goal}.</p></div>
      <div className="roadmap-vision-hotspots" aria-label="Roadmap destinations">
        {(Object.keys(roadmapLocationCopy) as RoadmapLocation[]).map(location => <button key={location} type="button" className={cn("roadmap-hotspot", `roadmap-hotspot-${location}`, selectedLocation === location && "roadmap-hotspot-selected")} onClick={() => setSelectedLocation(location)} aria-label={`Select ${roadmapLocationCopy[location].label}`}>{roadmapLocationCopy[location].label}</button>)}
      </div>
...
function LegacyRoadmapView({ onMission }: { onMission: () => void }) {
  const [selectedNode, setSelectedNode] = useState("What is code?");
  const [selectedWorldNumber, setSelectedWorldNumber] = useState("01");
  const { data: profile } = trpc.learner.profile.useQuery();
  const { data: learnerMissions } = trpc.learner.missions.useQuery();
  const currentMission = learnerMissions?.find(mission => mission.state !== "completed") ?? learnerMissions?.[0];
  const goal = profile?.careerGoal || "software engineering";
  const activeNode = currentMission?.title || selectedNode;
  const completedMissionCount = learnerMissions?.filter(mission => mission.state === "completed").length ?? 0;
  const activeWorldIndex = Math.min(4, completedMissionCount);
  const personalizedStages = roadmapStages.map((stage, index) => {
    const state = index < activeWorldIndex ? "completed" : index === activeWorldIndex ? "current" : index === activeWorldIndex + 1 ? "next" : "locked";
    const personalized = index === 0 ? { ...stage, subtitle: `Start here for ${goal}`, nodes: [currentMission?.title || "What is code?", "Python basics", "Git & GitHub"] } : index === 2 && goal.toLowerCase().includes("ai") ? { ...stage, title: "Web Development for intelligent products", subtitle: "Turn data into useful tools", nodes: ["HTML & CSS", "APIs", "Model basics", "Evaluation"] } : stage;
    return { ...personalized, state };
  });
  const selectedStage = personalizedStages.find(stage => stage.number === selectedWorldNumber) ?? personalizedStages[activeWorldIndex] ?? personalizedStages[0];
  const currentWorldProgress = currentMission?.progress ?? 0;
  return <>
    <WorldIntro kind="roadmap" onAction={onMission} />
    <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
      <div className="roadmap-fantasy-map space-y-4">{personalizedStages.map((stage, index) => <section key={stage.number} role="button" tabIndex={0} onClick={() => { setSelectedWorldNumber(stage.number); setSelectedNode(stage.nodes[0]); }} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedWorldNumber(stage.number); setSelectedNode(stage.nodes[0]); } }} className={cn("roadmap-world-card surface-card rounded-[24px] p-5 md:p-6", `roadmap-world-${index + 1}`, stage.state === "current" && "roadmap-world-current", stage.state === "locked" && "roadmap-world-locked", stage.state === "completed" && "roadmap-world-completed")}><div className="flex items-start gap-4"><div className={cn("roadmap-world-sigil flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl font-display font-bold", stage.state === "current" ? "bg-[#315d58] text-white" : stage.state === "next" ? "bg-[#f6ebe5] text-[#c27a67]" : "bg-[#f2ede7] text-[#b7aa9d]")}>{stage.number}</div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="roadmap-world-name">{stage.world}</p><h2 className="font-display text-xl font-semibold text-[#2d3c39]">{stage.title}</h2><p className="mt-1 text-sm text-[#8a7d70]">{stage.subtitle}</p></div><span className={cn("rounded-full px-3 py-1.5 text-xs font-bold", stage.state === "current" ? "bg-[#e4f0e8] text-[#5b937d]" : stage.state === "next" ? "bg-[#fff0e8] text-[#c27b68]" : "bg-[#f5f0eb] text-[#ac9f91]")}>{stage.state === "current" ? "In focus" : stage.state === "next" ? "Next up" : stage.state === "completed" ? "Complete" : "Locked"}</span></div><div className="mt-5 flex items-center gap-3"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[#eee8df]"><div className={cn("h-full rounded-full", stage.state === "completed" ? "bg-[#6ca595]" : stage.state === "current" ? "bg-[#e9917d]" : "bg-[#c5bbb0]")} style={{ width: `${stage.state === "completed" ? 100 : stage.state === "current" ? currentWorldProgress : 0}%` }} /></div><span className="text-[11px] font-bold text-[#9a8d80]">{stage.state === "completed" ? "Complete" : stage.state === "current" ? `${currentWorldProgress}%` : "Not open"}</span></div><div className="mt-4 flex flex-wrap gap-2">{stage.nodes.map(node => <button key={node} onClick={event => { event.stopPropagation(); if (stage.state !== "locked") setSelectedNode(node); }} aria-disabled={stage.state === "locked"} className={cn("roadmap-node", (selectedNode === node || activeNode === node) && stage.state !== "locked" && "roadmap-node-active", stage.state === "locked" && "cursor-not-allowed opacity-55")}><span className={cn("mr-2 inline-block h-2 w-2 rounded-full", stage.state === "current" ? "bg-[#6ca595]" : stage.state === "next" ? "bg-[#e9917d]" : "bg-[#c5bbb0]")} />{node}{stage.state === "locked" && <LockKeyhole className="ml-2 h-3.5 w-3.5" />}</button>)}</div></div></div></section>)}</div>
      <aside className="surface-card h-fit rounded-[24px] p-6 xl:sticky xl:top-7"><div className="flex items-center justify-between"><p className="eyebrow">{selectedStage.state === "locked" || selectedStage.state === "next" ? "World preview" : "Your next step"}</p><HanaAvatar small mood={selectedStage.state === "locked" ? "listening" : "encouraging"} /></div><h2 className="mt-3 font-display text-2xl font-semibold text-[#2d3c39]">{selectedStage.world}</h2><p className="mt-2 text-sm font-semibold text-[#5f6e69]">{selectedStage.title}</p><div className="mt-3 flex items-center gap-2 text-sm text-[#6ca595]"><span className={cn("h-2.5 w-2.5 rounded-full", selectedStage.state === "locked" ? "bg-[#c5bbb0]" : "bg-[#6ca595]")} />{selectedStage.state === "locked" || selectedStage.state === "next" ? "Preview only" : selectedStage.state === "completed" ? "Complete" : "In focus"}</div><div className="my-6 h-px bg-[#eee4d8]" /><p className="text-sm font-semibold text-[#4d5e59]">This world teaches</p><p className="mt-2 text-sm leading-6 text-[#76695d]">{selectedStage.teaches}</p><p className="mt-5 text-sm font-semibold text-[#4d5e59]">Learning path</p><ol className="mt-3 space-y-3 text-sm text-[#76695d]">{selectedStage.path.map((item, index) => <li key={item} className="flex gap-3"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#e8f1ea] text-[10px] font-bold text-[#5d947f]">{index + 1}</span>{item}</li>)}</ol><div className="mt-5 rounded-2xl bg-[#fff6ee] p-4"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#b58d74]">Final project</p><p className="mt-2 text-sm font-semibold text-[#66594e]">{selectedStage.project}</p><p className="mt-3 text-xs leading-5 text-[#806b5f]">Unlock requirement: {selectedStage.unlock}</p></div>{selectedStage.state === "current" ? <Button onClick={onMission} className="mt-6 w-full rounded-full bg-[#315d58] hover:bg-[#254c48]">Continue Mission <ArrowRight className="ml-2 h-4 w-4" /></Button> : <Button onClick={() => setSelectedWorldNumber(selectedStage.number)} className="mt-6 w-full rounded-full border border-[#e7ddd2] bg-[#fffaf4] text-[#76695d] hover:bg-[#faf5ef]">View Preview</Button>}</aside>
    </div>
  </>;
}
...
function getProjectResources(skills: string[]): RoadmapResource[] {
  const skillText = skills.join(" ").toLowerCase();
  const keys = skillText.includes("python") || skillText.includes("ai") ? ["python", "aiEngineering", "projectPractice"] : skillText.includes("javascript") || skillText.includes("react") ? ["javascript", "react", "projectPractice"] : skillText.includes("api") || skillText.includes("node") ? ["rest", "node", "projectPractice"] : ["programming", "git", "projectPractice"];
  const items = keys.flatMap(key => masterResourceLibrary[key] ?? []);
  const selected = [items.find(item => item.role === "primary"), items.find(item => item.role === "practice"), items.find(item => item.role === "project")].filter((item): item is (typeof items)[number] => Boolean(item));
  return selected.slice(0, 3).map(item => ({ title: item.title, source: new URL(item.url).hostname.replace(/^www\\./, ""), format: item.role === "primary" ? "Learn" : item.role === "practice" ? "Practice" : "Build proof", minutes: item.role === "primary" ? "Start here" : item.role === "practice" ? "Try it" : "Show your work", url: item.url, why: item.role === "project" ? "Turn this project checkpoint into visible evidence." : "Use this resource to make the next project checkpoint easier." }));
}
...
function ProjectsView({ onChat }: { onChat: () => void }) {
  ...
  const updateProject = trpc.learner.updateProjectCheckpoint.useMutation();
  const savePortfolioDraft = trpc.learner.savePortfolioDraft.useMutation();
  const utils = trpc.useUtils();
  const projects = projectRecords && projectRecords.length > 0 ? projectRecords.map((project, index) => ({ id: project.id, title: project.title, desc: project.description || "A learner-owned project that turns practice into visible evidence.", progress: project.progress, skills: project.skills ? project.skills.split(",").map(skill => skill.trim()).filter(Boolean) : ["Practice", "Evidence"], next: project.currentCheckpoint || "Choose one small checkpoint", tone: index % 2 === 0 ? "coral" : "mint", resources: getProjectResources(project.skills ? project.skills.split(",").map(skill => skill.trim()).filter(Boolean) : ["Practice", "Evidence"]) })) : demoProjects.map(project => ({ ...project, id: undefined, resources: getProjectResources(project.skills) }));
  return <><WorldIntro kind="projects" onAction={() => document.getElementById("current-project-quest")?.scrollIntoView({ behavior: "smooth", block: "center" })} /><div className="mb-5 grid gap-4 md:grid-cols-3"><div className="surface-card rounded-[22px] p-5"><p className="eyebrow">In progress</p><p className="mt-3 font-display text-3xl font-semibold text-[#2d3c39]">{projects.filter(project => project.progress < 100).length}</p><p className="mt-1 text-sm text-[#8a7d70]">projects with a next step</p></div><div className="surface-card rounded-[22px] p-5"><p className="eyebrow">Evidence captured</p><p className="mt-3 font-display text-3xl font-semibold text-[#2d3c39]">{projects.reduce((total, project) => total + (project.progress > 0 ? 1 : 0), 0)}</p><p className="mt-1 text-sm text-[#8a7d70]">projects with evidence started</p></div><div className="surface-card rounded-[22px] p-5"><p className="eyebrow">Portfolio pieces</p><p className="mt-3 font-display text-3xl font-semibold text-[#2d3c39]">{projects.filter(project => project.progress >= 70).length}</p><p className="mt-1 text-sm text-[#8a7d70]">ready for your review</p></div></div><div className="grid gap-5 lg:grid-cols-2">{projects.map((project, index) => <section key={project.title} id={index === 0 ? "current-project-quest" : undefined} className="surface-card rounded-[26px] p-6 md:p-7"><div className="flex items-start justify-between gap-4"><div><span className={cn("pill", project.tone === "coral" ? "pill-coral" : "pill-sage")}>{project.progress}% built</span><h2 className="mt-4 font-display text-2xl font-semibold text-[#2d3c39]">{project.title}</h2><p className="mt-2 text-sm leading-6 text-[#76695d]">{project.desc}</p></div><button className="icon-button"><MoreHorizontal className="h-4 w-4" /></button></div><div className="mt-6"><div className="mb-2 flex justify-between text-xs font-semibold text-[#a09283]"><span>Checkpoint progress</span><span>{project.progress}%</span></div><ProgressBar value={project.progress} tone={project.tone === "coral" ? "coral" : "sage"} /></div><div className="mt-6 flex flex-wrap gap-2">{project.skills.map(skill => <span key={skill} className="tag">{skill}</span>)}</div><div className="mt-6 rounded-2xl bg-[#fff7ee] p-4"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#b58d74]">Next checkpoint</p><p className="mt-2 text-sm font-semibold text-[#66594e]">{project.next}</p></div><div className="mt-6 rounded-2xl border border-[#e8ded2] bg-[#fffdf9] p-4"><div className="flex items-center justify-between gap-3"><p className="eyebrow">Guild field kit</p><span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#9a8b7c]">3 focused resources</span></div><div className="mt-3 grid gap-2 md:grid-cols-3">{project.resources.map(resource => <a key={resource.title} href={resource.url} target="_blank" rel="noreferrer" className="group rounded-xl border border-[#eee4d8] bg-[#fffaf4] p-3 transition hover:-translate-y-0.5 hover:border-[#b8d2c5] hover:bg-[#f7fcf8]"><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#6ca595]">{resource.format}</p><p className="mt-1 line-clamp-2 text-sm font-semibold leading-5 text-[#3d514b]">{resource.title}</p><p className="mt-2 text-[11px] font-medium text-[#9a8b7c]">{resource.minutes} · Open <ArrowUpRight className="ml-0.5 inline h-3 w-3" /></p></a>)}</div><p className="mt-3 text-xs leading-5 text-[#8a7d70]">Learn one idea, practice it, then return here with evidence for your next checkpoint.</p></div><div className="mt-6 flex flex-wrap gap-2"><Button onClick={() => { if (project.id) { const nextProgress = Math.min(100, project.progress + 10); const nextCheckpoint = project.progress >= 90 ? "Portfolio-ready checkpoint" : project.next; updateProject.mutate({ projectId: project.id, progress: nextProgress, currentCheckpoint: nextCheckpoint, status: nextProgress >= 100 ? "completed" : "active" }, { onSuccess: () => { if (nextProgress >= 100) savePortfolioDraft.mutate({ projectId: project.id, kind: "portfolio", content: `Portfolio draft for ${project.title}\n\nWhat I built: ${project.desc}\nSkills practiced: ${project.skills.join(", ")}\nFinal checkpoint: ${nextCheckpoint}\n\nThis draft is ready for learner review and editing.` }, { onSuccess: () => { utils.learner.projects.invalidate(); utils.learner.portfolioDrafts.invalidate(); } }); } }); } }} disabled={updateProject.isPending || savePortfolioDraft.isPending || project.progress >= 100} className="rounded-full bg-[#315d58] hover:bg-[#254c48]">{project.progress >= 100 ? "Checkpoint complete" : "Continue building"} <ArrowRight className="ml-2 h-4 w-4" /></Button><button onClick={onChat} className="rounded-full border border-[#e7ddd2] px-4 py-2 text-sm font-semibold text-[#76695d]">Ask Hana</button></div></section>)}</div>{projectsLoading && <p className="mt-4 text-xs font-semibold text-[#a09283]">Loading your saved project checkpoints…</p>}<section className="mt-5 rounded-[26px] border border-dashed border-[#dbcfc1] bg-[#fcf8f2] p-6"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div className="flex items-start gap-4"><div className="rounded-2xl bg-[#e7f1ea] p-3 text-[#60967e]"><Sparkles className="h-5 w-5" /></div><div><p className="eyebrow">Recommended for your roadmap</p><h2 className="mt-2 font-display text-xl font-semibold text-[#2d3c39]">Build a CLI habit tracker</h2><p className="mt-1 text-sm text-[#76695d]">Practice Python, file persistence, and clean README writing.</p></div></div><button className="rounded-full border border-[#d9cdbf] px-4 py-2 text-sm font-semibold text-[#6b7a73] hover:bg-white">Preview idea <ArrowRight className="ml-1 inline h-4 w-4" /></button></div></section></>;
}

function getOpportunityResources(type: string): RoadmapResource[] {
  const normalized = type.toLowerCase();
  const keys = normalized.includes("hack") ? ["hackathons", "projectPractice"] : normalized.includes("open") || normalized.includes("source") ? ["openSource", "github"] : normalized.includes("career") || normalized.includes("fellow") ? ["career", "communication"] : ["projectPractice", "career"];
  const items = keys.flatMap(key => masterResourceLibrary[key] ?? []);
  const selected: typeof items = [];
  for (const role of ["primary", "practice", "project", "evidence", "advanced"] as const) {
    const item = items.find(candidate => candidate.role === role && !selected.some(existing => existing.url === candidate.url));
  ...
  return selected.map(item => ({ title: item.title, source: new URL(item.url).hostname.replace(/^www\\./, ""), format: item.role === "primary" ? "Start here" : item.role === "practice" ? "Practice" : item.role === "project" ? "Build proof" : "Evidence", minutes: item.role === "primary" ? "Learn" : item.role === "practice" ? "Try it" : "Open next", url: item.url, why: item.role === "project" || item.role === "evidence" ? "Turn this opportunity into visible career evidence." : "Use this to prepare with a focused next step." }));
}
...
function ChatView() {
  ...
  const activeProject = chatProjects?.find(project => project.status !== "completed") ?? chatProjects?.[0];
  const activeMission = chatMissions?.find(mission => mission.state !== "completed") ?? chatMissions?.[0];
  const learnerGoal = profile?.careerGoal || "Software engineer";
  const learnerFocus = activeMission?.title || "REST APIs";
  const learnerProject = activeProject?.title || "Weather app";
  const learnerStyle = profile?.learningStyle || "Examples first";
